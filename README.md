...
| AI 助手 | 支持基于已配置模型的连续对话、资料库查询和联网搜索；未配置模型时，聊天空状态保持简洁，模型与供应商配置可从右上角入口进入。 |

## AI 助手

AI 助手用于把资料库中的内容、联网搜索结果与当前对话放到同一个工作流中。首次使用时，点击右上角钥匙图标即可配置内置或自定义供应商，并选择可用模型。

- **连续对话**：会话会自动保存，可新建、切换或删除；流式回复会展示处理进度与本次耗时。
- **资料库协作**：可检索和读取资料库中的文档、文本笔记与链接，也可查看和整理收集箱内容。图片暂不支持文字识别。
- **联网搜索**：可通过必应或 AnySearch 查询时效信息，并在需要时继续阅读搜索结果中的网页内容。
- **资料库管理**：可保存笔记、归档或移除收集箱内容、重命名和删除资料库条目、更新条目笔记。
- **文件生成**：可将整理结果生成 Word、Excel、Markdown 或 PDF，并自动加入资料库；生成新文件不会覆盖原始资料。
- **操作授权**：任何会写入、修改或删除资料库的操作都会在对话中请求确认，只有明确允许后才会执行。

联网搜索需要网络连接。对话内容和需要处理的资料会发送给所配置的模型供应商，请勿提交不应外发的敏感信息。助手不会执行系统命令、脚本或其他系统操作。
